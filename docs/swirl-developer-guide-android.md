...
The current version is compiled with compileSdkVersion 26.  The current targetSdkVersion is 23.
...
```
#### Startup Options
There are several ways to pass startup options to the Swirl library.  You can do so when you call start by passing a Bundle of options in.

```java
        Bundle options = new Bundle();
        options.putString(Settings.NOTIFICATION_CHANNEL, "my_notification_channel_id");
        ...
```
 
You can also use the string representation of the option added to the application meta-data within the AndroidManifest.xml.  Note that all such keys are prefixed with `com.swirl`.  See below for an example using API_KEY.
...
<application
  ...
  <service  android:name="com.swirl.Swirl$Service" android:permission="android.permission.BIND_JOB_SERVICE"/>
...
#### Changes for Android 8.0

If you are targeting API 26 and above, please note that you will need to provide a valid notification channel ID to be used for all notifications generated by the SDK.  You pass that in as in the above example using Settings.NOTIFICATION_CHANNEL.
You also need to add the permission.BIND_JOB_SERVICE when you register the service in the Manifest.

If you do not do these things, then the SDK will not function properly in the background.
